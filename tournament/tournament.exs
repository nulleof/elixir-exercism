defmodule Tournament do
  ...
  def gen_result_map(lines) do
    result_table = Enum.reduce(lines, %{}, fn(x, acc) -> add_line_to_map(acc, x) end)
    IO.inspect result_table
  end
  ...
  def update_team_results(old_results = %{W: w_num, L: l_num, D: d_num}, action) do
    case action do
      :win -> %{old_results | W: w_num + 1}
      :lose -> %{old_results | L: l_num + 1}
      :draw -> %{old_results | D: d_num + 1}
    end
  end
  ...
  def add_line_to_map(map, line) do
    case String.split(line, ";") |> IO.inspect do
